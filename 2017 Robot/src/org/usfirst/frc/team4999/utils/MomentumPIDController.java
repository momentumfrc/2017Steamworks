 ...
 */
public class MomentumPIDController implements PIDInterface, LiveWindowSendable, Controller {
	...
	public class PercentageTolerance implements Tolerance {
		...
		public boolean onTarget() {
			return isAvgErrorValid()
					&& (Math.abs(getAvgError()) < m_percentage / 100 * (m_maximumInput - m_minimumInput));
		}
	}
	...
	private class PIDTask extends TimerTask {
		...
		private MomentumPIDController m_controller;

		public PIDTask(MomentumPIDController controller) {
			if (controller == null) {
				throw new NullPointerException("Given MomentumPIDController was null");
			}
	 ...
	 * @param period
	 *            the loop time for doing calculations. This particularly effects
	 *            calculations of the integral and differential terms. The default
	 *            is 50ms.
	 */
	@SuppressWarnings("ParameterName")
	public MomentumPIDController(double Kp, double Ki, double Kd, double Kf, PIDSource source, PIDOutput output,
			double period) {
	 ...
	 */
	@SuppressWarnings("ParameterName")
	public MomentumPIDController(double Kp, double Ki, double Kd, PIDSource source, PIDOutput output, double period) {
	 ...
	 */
	@SuppressWarnings("ParameterName")
	public MomentumPIDController(double Kp, double Ki, double Kd, PIDSource source, PIDOutput output) {
		...
		if (enabled) {
			...
			synchronized (this) {
				...
				if (m_pidInput.getPIDSourceType().equals(PIDSourceType.kRate)) {
					...
					if (m_I != 0) {
						if (Math.abs(m_error) > 10) // Dump large error integrals to avoid windup
							m_totalError = 0;
	 ...
	 * because the MomentumPIDController class only calls it in synchronized code,
	 * so be careful if calling it oneself.
	 *
	 * <p>
	...
	/**
	 * Set the setpoint for the MomentumPIDController Clears the queue for
	 * GetAvgError().
	 *
	 * @param setpoint
	...
	/**
	 * Returns the current setpoint of the MomentumPIDController.
	 *
	...
	/**
	 * Returns the change in setpoint over time of the MomentumPIDController.
	 *
	...
	/**
	 * Returns whether or not any values have been collected. If no values have been
	 * collected, getAvgError is 0, which is invalid.
	 *
	 ...
	 * @deprecated Use {@link #setPercentTolerance} or {@link #setAbsoluteTolerance}
	 *             instead.
	 */
	@Deprecated
	 ...
	 * MomentumPIDController.AbsoluteTolerance(0.1))
	 *
	 * @param tolerance
	...
	/**
	 * Begin running the MomentumPIDController.
	 */
	@Override
	...
	/**
	 * Stop running the MomentumPIDController, this sets the output to zero before
	 * stopping.
	 */
	@Override
	...
	/**
	 * Return true if MomentumPIDController is enabled.
	 *
